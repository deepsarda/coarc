...
export async function POST(request: Request) {
	try {
		...
		// Rate limit
		const rateCheck = await checkRateLimit(admin, user.id, 'send_duel', RATE_LIMITS.SEND_DUEL);
		...
		const avgRating = Math.round(
			((challengerRating?.new_rating ?? 1200) + (challengedRating?.new_rating ?? 1200)) / 2,
		);
		...
		if (!problem) {
			return NextResponse.json({ error: 'No suitable problem found for duel' }, { status: 404 });
		}
