...
export async function POST() {
	try {
		...
		if (!profile.cf_handle && !profile.lc_handle) {
			return NextResponse.json({ error: 'No handles linked', cf: null, lc: null }, { status: 400 });
		}

		// Run the sync
		const result = await syncUserStats(admin, user.id, profile.cf_handle, profile.lc_handle);
