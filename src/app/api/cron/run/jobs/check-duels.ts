...
export async function runCheckDuels(admin: SupabaseClient) {
	...
	for (const duel of activeDuels ?? []) {
		...
		const expiresAt = duel.expires_at
			...
			: new Date(
					new Date(duel.started_at).getTime() +
						duel.time_limit_minutes * 60 * 1000,
				);
		...
		const challengerHandle = participants.find(
			(p) => p.id === duel.challenger_id,
		)?.cf_handle;
		const challengedHandle = participants.find(
			(p) => p.id === duel.challenged_id,
		)?.cf_handle;
		...
		try {
			...
			for (const sub of challengerSubs) {
				if (
					...
					challengerSolveTime = Math.round(
						(sub.submittedAt.getTime() - startTime) / 1000,
					);
					break;
				}
			}
			...
			for (const sub of challengedSubs) {
				if (
					...
					challengedSolveTime = Math.round(
						(sub.submittedAt.getTime() - startTime) / 1000,
					);
					break;
				}
		...
		const bothSolved =
			challengerSolveTime !== null && challengedSolveTime !== null;
		const anySolved =
			challengerSolveTime !== null || challengedSolveTime !== null;
		...
		if (bothSolved || isExpired) {
			...
			if (challengerSolveTime !== null && challengedSolveTime !== null) {
				winnerId =
					challengerSolveTime <= challengedSolveTime
						? duel.challenger_id
						: duel.challenged_id;
